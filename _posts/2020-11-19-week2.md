...
- 타입 변환 (type conversion)
  ...
  타입 변환은 크게 묵시적 타입 변환(자동 형 변환) 과 명시적 타입 변환(강제 형 변환) 두 가지로 구별할 수 있는데, 타입 변환의 기본 원칙은 다음과 같다.
...
- 업캐스팅과 다운 캐스팅
  ...
  public class Main {
    public static void main(String args[])
    {
      ...
      Dog dog_b = new Animal(); // 다운캐스팅으로 보이지만 컴파일 에러
  ...
  ​		Dog dog2 = new Animal(); // 컴파일 에러 (다운 캐스팅 X);
...
https://medium.com/plants-studio/%EC%95%8C%EC%95%84%EB%91%90%EB%A9%B4-%EC%93%B8%EB%AA%A8%EC%9E%88%EB%8A%94-%EC%9E%90%EB%B0%94%EC%9D%98-%ED%97%B7%EA%B0%88%EB%A6%AC%EB%8A%94-%EC%A0%90-1-26b1e1a35491 [블로그]
